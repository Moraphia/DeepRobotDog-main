...
class HandGesture:
    ...
    def visualize(self, image, thickness=2):
        for idx, handpose in enumerate(self.hands):
            ...
            if self.gestures[idx] is not None:
                image = self.cv2ImgAddText(image, '{}'.format(self.gestures[idx]), bbox[0],
                                           bbox[1] + -15 * thickness, )
    ...
    def __gesture_classification(self, hand):
        ...
        # twisting
                ...
                firstOpen and not secondOpen and not thirdOpen and fourthOpen:
            gesture_str = "原地扭身"
        # forward
        elif (angle_list[0] > thr_angle_thumb) and (angle_list[1] < thr_angle_s) and (
        ...
        # stop
            ...
            gesture_str = "左旋转"
        elif (angle_list[0] < thr_angle_s) and (angle_list[1] > thr_angle_x) and (
                angle_list[2] > thr_angle) and (
                angle_list[3] > thr_angle) and (angle_list[4] > thr_angle) and \
                thumbOpen  and not secondOpen and not thirdOpen and not fourthOpen:
            gesture_str = "前进"
        return gesture_str